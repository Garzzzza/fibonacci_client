...
function showAlert() {
    ...
    grabInput.setAttribute("class", "mx-2 inputfield align-self-start alert alert-danger");
}
function hideAlert() {
    alerty.removeAttribute("class");
...
function serverCalculator() {
    fetch(`http://localhost:5050/fibonacci/${grabInput.value}`)
            ...
            if (response.ok) {
                ...
                response.json().then(function (data) {
                    ...
                    for (let i = list.length - 6; i < list.length; i++) {
                        html += ` <p class="my-2  border-dark border-bottom">
The Fibonnaci Of <span class="fw-bold">`+ list[i].number + `</span> is <span class="fw-bold">` + list[i].result + `</span>. Calculated at: ` + new Date(list[i].createdDate) + `
</p>`
                    }
